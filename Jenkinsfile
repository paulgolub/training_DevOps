    ...
    node () {
		...
		if(tomcats.size()>0){
			...
			for (tomcat in tomcats){
				stage("Deploy WAR-artifact to "+tomcat.toString()+""){
					...
					withCredentials([usernamePassword(credentialsId: 'tomcatcredentials', passwordVariable: 'Tom_Pass', usernameVariable: 'Tom_U')]) {
		...
		stage('Push to GIT') {
			withCredentials([usernamePassword(credentialsId: 'gitcredentials', passwordVariable: 'git_pass', usernameVariable: 'git_login')]) {
				...
				sh("git status")
				sh("git tag ${version}")
				sh("git commit -am \"Jenkins auto commit. Build - ${version}.${BUILD_NUMBER}\"")
				sh("git push --tags https://${git_login}:${git_pass}@github.com/${gitUrl} ${tree}")
				sh("git checkout master")
				sh("git merge task3_1")
